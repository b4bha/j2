...
import javax.swing.*;
import java.io.*;
class Slip12_1 extends JFrame implements ActionListener,ItemListener
{
	...
	/*		else 
if(s.equals("Sum")){t.setText(new Integer(givesum()).toString());}else if(s.equals("Average")){t.setText(new Double(giveavg()).toString());}else if(s.equals("Maximum")){t.setText(new Integer(givemax()).toString());}else if(s.equals("Minimum")){t.setText(new Integer(givemin()).toString());}else if(s.equals("Median")){t.setText(new Integer(givemed()).toString());}*/
	...
	// else if(s.equals("Search")){int ser=Integer.parseInt(JOptionPane.showInputDialog("Enter Number To Search"));int a=0;for(int i=0;i<n;i++){if(arr[i]==ser){t.setText("Number Present");a=1;}if(a==0)t.setText("Number Not Present");}}}public int givesum(){int sum=0;for(int i=0;i<n;i++){sum=sum+arr[i];}return sum;}public double giveavg(){int sum=0;for(int i=0;i<n;i++){sum=sum+arr[i];}return sum/n;}public int givemax(){int sum=arr[0];for(int i=0;i<n;i++){if(arr[i] > sum)sum=arr[i];}return sum;}public int givemin(){int sum=arr[0];for(int i=0;i<n;i++){if(arr[i] < sum)sum=arr[i];}return sum;}public int givemed(){return arr[(n/2)];}
	}									 							
	
	public static void main(String arg[])
	{
		Slip12_1 c =new Slip12_1("My Computation");
		c.setSize(300,150);
		c.setVisible(true);
